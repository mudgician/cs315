...
class HMM(object):
    ...
    >>> print("%.8f" % ll)
    2.90534116
    >>> print("%.8f" % hmm.score(signal, vals))
    2.90534116
    ...
    def _viterbi(signal, trans, dists):
        ...
        sigma = np.zeros((signal.shape[1], n_states))
        seq = np.zeros((signal.shape[1], n_states), dtype=int)
        
        # initialization
        obs = signal[:, 0]
        for i in range(n_states):
            sigma[0][i] = dists[i].logf(obs) + np.log(trans[-1][i])
            
        # running
        for sig in range(1, signal.shape[1]):
            obs = signal[:, sig]
            for c_state in range(n_states):
                l_max = np.log(0)
                c_seq = 0
                for p_state in range(n_states):
                    something = sigma[sig-1][p_state] + np.log(trans[p_state][c_state])
                    if(something > l_max):
                        l_max = something
                        c_seq = p_state
                sigma[sig][c_state] = l_max + dists[c_state].logf(obs)
                seq[sig][c_state] = c_seq
            
        # termination
        best_final_score = np.log(0)
        best_last_state = 0
        for c_state in range(n_states):
            # Score at the last time step + transition to the final non-emitting state
            final_score = sigma[-1][c_state] + np.log(trans[c_state][-1])
            
            if final_score > best_final_score:
                best_final_score = final_score
                best_last_state = c_state

        # backtracking
        path = np.zeros((signal.shape[1]), dtype=int)
        current = best_last_state
        
        # Safely lock in the final state at the very end of the array
        path[-1] = current
        
        for i in range(signal.shape[1] - 1, 0, -1):
            prev = int(seq[i][current])
            # Shift by 1 to avoid overwriting the end of the array
            path[i - 1] = prev
            current = prev
    ...
    def _score(signal, seq, trans, dists):
        ...
        for i in range(seq.shape[0]):
            total += dists[seq[i]].loglike(signal[:,i])
            total += trans[seq[i]][seq[i+1]]
        '''
   
        # Start with 0.0
        total = 0.0

        # 1. The Start Bookend: Transition from initial state (last row) to the first state in seq
        #-1 is the last row
        total += np.log(trans[-1, seq[0]])
        seq = np.array(seq)
        signal = np.array(signal)
        for i in range(seq.shape[0]):
    
            # 2. Add the emission log-probability
            # Grab the i-th observation (column) from the signal
            obs = signal[:, i]
            total += dists[seq[i]].logf(obs)
    
            # 3. Transition to the next state (only if we aren't at the very end of the sequence)
            if i < seq.shape[0] - 1:
                total += np.log(trans[seq[i], seq[i+1]])

        # 4. The End Bookend: Transition from the last state in seq to the final state (last column)
        total += np.log(trans[seq[-1], -1])
    ...
    def _forward(signal, trans, dists):
        ...
        for i in range(n_states):
            # np.log(Start -> State i) + Emission of State i
            alpha[i] = np.log(trans[-1, i]) + dists[i].logf(obs)
        
        # -------------------------
        # 2. RECURSION (t = 1 to n_obs - 1)
        # -------------------------
        for t in range(1, n_obs):
            obs = signal[:, t]
            new_alpha = np.zeros(n_states)
        
            # Calculate the new score for each state 'k'
            for k in range(n_states):
                
                # Collect all the incoming paths to state 'k'
                incoming_paths = []
                for j in range(n_states):
                    # Previous score of 'j' + np.log(Transition j -> k)
                    path_score = alpha[j] + np.log(trans[j, k])
                    incoming_paths.append(path_score)
                
                # Safely SUM the incoming paths, then ADD the current emission
                new_alpha[k] = np.logaddexp.reduce(incoming_paths) + dists[k].logf(obs)
            
            # Move forward in time! Update alpha for the next loop.
            alpha = new_alpha
        ...
        final_paths = []
        for j in range(n_states):
            # Current score of 'j' + np.log(Transition j -> Final State)
            path_score = alpha[j] + np.log(trans[j, -1])
            final_paths.append(path_score)
        
        # Return the total combined likelihood of all paths
        return np.logaddexp.reduce(final_paths)
    ...
    def _calcstates(self, trans, dists):
        ...
        for sig in self.signals:
            ...
            total_ll += ll
            sig_states = np.zeros((sig.shape[1], n_states), dtype=bool)
            for t, p in enumerate(path):
                sig_states[t, p] = True
            states_list.append(sig_states)
            
        states = np.vstack(states_list)
        used_states = np.any(states, axis=0)
        states = states[:, used_states]
        
        n_states_prime = np.sum(used_states)
        new_trans = np.zeros((n_states_prime + 1, n_states_prime + 1))
        
        start_obs_indices = np.cumsum([0] + [s.shape[1] for s in self.signals])[:-1]
        start_states = states[start_obs_indices, :]
        new_trans[-1, :-1] = np.sum(start_states, axis=0)
        start_sum = np.sum(new_trans[-1, :-1])
        if start_sum > 0:
            new_trans[-1, :-1] /= start_sum
            
        obs_idx = 0
        for sig in self.signals:
            n_obs_i = sig.shape[1]
            for t in range(n_obs_i - 1):
                curr_st = np.where(states[obs_idx + t, :])[0][0]
                next_st = np.where(states[obs_idx + t + 1, :])[0][0]
                new_trans[curr_st, next_st] += 1.0
            
            last_st = np.where(states[obs_idx + n_obs_i - 1, :])[0][0]
            new_trans[last_st, -1] += 1.0
            obs_idx += n_obs_i
            
        for i in range(n_states_prime):
    ...
    def _updatecovs(self, states):
        ...
        for i in range(n_states):
            state_obs = self.data[:, states[:, i]]
            n_k = state_obs.shape[1]
            
            if n_k == 0:
                means[i, :] = np.zeros(d)
                covs[i, :, :] = np.eye(d)
            else:
                means[i, :] = np.mean(state_obs, axis=1)
                if n_k > 1:
                    cov_k = np.cov(state_obs, bias=True)
                    if cov_k.ndim == 0:
    ...
    def _em(self, trans, states):
        ...
        while not converged and iters <  self.maxiters:
            ...
            if oldLL != 0 and abs((newLL - oldLL) / oldLL) < self.rtol:
                converged = True
        ...
        return trans, dists, oldLL, iters
    ...
    def sample(self):
        ...
        # this thing is for state transitions
        ...
        # The final non-emitting state is the last row/column index
        final_state = self.trans.shape[0] - 1
        
        # Get the initial state by sampling from the start probabilities (the last row)
        ...
        while current_state != final_state:
            ...
            # 2. Draw a sample from the Gaussian for the current state
            # Gaussian.sample() returns a 2D array like [[0.837]], we flatten it to [0.837] for the list
            obs = self.dists[current_state].sample().flatten()
            samples.append(obs)
            
            # 3. Transition to the next state using the current state's row of transition probabilities
