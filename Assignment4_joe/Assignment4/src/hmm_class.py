...
class HMM(object):
    ...
    def _viterbi(signal, trans, dists):
        ...
        N = trans.shape[0] - 1  # number of emitting states
        T = signal.shape[1]  # number of observations

        # delta[j,t] = log probability of best path ending in state j at time t
        delta = np.full((N, T), -np.inf)

        # psi[j,t] = previous state that maximized delta
    ...
    def _score(signal, seq, trans, dists):
        ...
        for i in range(len(seq)):
            current_score += np.log(trans[prev_state,seq[i]])
            current_score += dists[seq[i]].loglik(signal[:,i])
            prev_state = seq[i]

        current_score += np.log(trans[prev_state,-1])
    ...
    def _forward(signal, trans, dists):
        ...
        for t in range(1, T):
            for j in range(N):
                ...
                for i in range(N):
                    if trans[i, j] > 0:
                        prev_terms.append(
                            log_alpha[i, t - 1] + np.log(trans[i, j])
                        )

                log_alpha[j, t] = (
                        np.logaddexp.reduce(prev_terms) +
                        dists[j].loglik(signal[:, t])
                )
        ...
        for j in range(N):
            if trans[j, -1] > 0:
                final_terms.append(
                    log_alpha[j, T - 1] + np.log(trans[j, -1])
                )

        return np.logaddexp.reduce(final_terms)
    ...
    def sample(self):
        ...
        current_state = draw_discrete_sample(self.trans[self.trans.shape[0] - 1, :])
        new_sample = 0
        samples = []
        states = []
        while current_state != self.trans.shape[0] - 1 :
            states.append(current_state)
            new_sample = self.dists[current_state].sample(n=1)
            samples.append(new_sample)

            current_state = draw_discrete_sample(self.trans[current_state,:])

        return np.asarray(samples), np.asarray(states)
